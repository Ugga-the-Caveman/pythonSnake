...
class BrainClass:
    ...
    def load(self, fileName):
        ...
        self.inputWdth = int(findParameter(fileName, "inputWdth", 15))
        self.inputHgth = int(findParameter(fileName, "inputHgth", 15))
    ...
    # einen teil der weigths neu machen
    def mutate(self, percentage):
        for x in range(len(self.lyrs)):
            for y in range(len(self.lyrs[x])):
                for z in range(len(self.lyrs[x][y])):

                    if percentage > random.uniform(0, 1):
                        self.lyrs[x][y][z] = random.uniform(-1, 1)
# ---
